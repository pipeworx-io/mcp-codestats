...
`tools/list` at `https://gateway.pipeworx.io/codestats/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/codestats_user \
  -H 'Content-Type: application/json' \
  -d '{"username":"johndoe"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/codestats_user`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
